...
// shEvaluate is equivalent to r2s Ambient variable.
...
float3 sphericalHarmonics(float3 L, float3 Color, float roughness)
{
	int samplesPerAxis = 16;
	int sampleCount = samplesPerAxis * roughness;
	...
	for (int i = 0; i < samplesPerAxis; i++)
		...
		{
			float azimuthX = (float)i / samplesPerAxis;
			float zenithY = (float)j / samplesPerAxis;

			float3 rayDir = shGetUniformSphereSample(azimuthX, zenithY);

			float3 color = Color;

			sh2 basis = shEvaluate(rayDir);
			shR = shAdd(shR, shScale(basis, color.r));
			shG = shAdd(shG, shScale(basis, color.g));
			shB = shAdd(shB, shScale(basis, color.b));
		}

	float shFactor = 4.0f * shPI / sampleCount;
	shR = shScale(shR, shFactor);
	shG = shScale(shG, shFactor);
	shB = shScale(shB, shFactor);

	float3 rgbColor = max(0.0f, shUnproject(shR, shG, shB, L));
