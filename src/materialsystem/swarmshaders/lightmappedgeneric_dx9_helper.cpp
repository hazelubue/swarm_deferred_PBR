...
// left here for my memory
// DYNAMIC: "DETAIL_BLEND_MODE"			"0..12"
//
...
#include "include/lightmappedgeneric_deferred_vs30.inc"
#include "include/lightmappedgeneric_deferred_ps30.inc"
...
void InitLightmappedGeneric_DX9(CBaseVSShader* pShader, IMaterialVar** params, LightmappedGeneric_DX9_Vars_t& info)
{
	...
	{
		int nDetailBlendMode = (info.m_nDetailTextureCombineMode == -1) ? 0 :
			params[info.m_nDetailTextureCombineMode]->GetIntValue();
		// Clamp to valid range 0-3 since you're using only 4 modes
		nDetailBlendMode = clamp(nDetailBlendMode, 0, 3);
	...
	if (pShaderShadow || (!pContextData) || pContextData->m_bMaterialVarsChanged || pContextData->m_bNeedsCmdRegen || (hasFlashlight && !IsX360()))
	{
		...
		if (pShaderShadow || bNeedRegenStaticCmds)
		{
			...
			if (pShaderShadow)
			{
				...
				if (hasDetailTexture && nDetailBlendMode != 3)
				{
					pShaderShadow->EnableTexture(SHADER_SAMPLER12, true);
					// Determine which modes need SRGB
					bool bSRGBState = (nDetailBlendMode == 0); // Mode 0 (Mod2X) typically needs SRGB
					// Mode 1 (Additive) might also need SRGB depending on your implementation
					// Mode 2 (Mask) might need linear
					// Mode 3 (Override) is handled separately
...
// 				else
	...
	DYNAMIC_STATE
	{
		...
		if (bDeferredActive)
		{
			DynamicCmdsOut.BindTexture(pShader, SHADER_SAMPLER15, GetDeferredExt()->GetTexture_LightAccum(), 0);
			...
			DynamicCmdsOut.SetPixelShaderConstant(33, fl1);
		}
