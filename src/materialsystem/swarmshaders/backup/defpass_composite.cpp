	...
	VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData *pDeferredContext )
{
	...
	bool hasSSBump = bHasNormalTexture && params[info.SelfShadowedBumpFlag]->GetIntValue() && (params[info.SelfShadowedBumpFlag]->GetIntValue());
	...
	SHADOW_STATE
	{
		...
		pShaderShadow->EnableSRGBRead(SHADER_SAMPLER11, bUseSRGB);
		...
		int bumpmap_variant = (hasSSBump) ? 2 : bHasNormalTexture;
		...
		//pShader->PI_SetPixelShaderAmbientLightCube(11);
		...
		//pShader->PI_SetModulationPixelShaderDynamicState_LinearColorSpace(12);
	...
	DYNAMIC_STATE
	{
			...
			|| building_cubemaps.GetBool() )
		{
			...
			tmpBuf.BindTexture( pShader, SHADER_SAMPLER12, info.MRAOTEXTURE);

			// Getting the light state
			/*LightState_t lightState;
			pShaderAPI->GetDX9LightState(&lightState);*/

			//tmpBuf.BindStandardTexture(SHADER_SAMPLER11, TEXTURE_LIGHTMAP_BUMPED);
		...
		if (bGBufferNormal)
		{
			...
			else
			{
				if (hasSSBump)
				{
					pShaderAPI->BindStandardTexture(SHADER_SAMPLER1, TEXTURE_SSBUMP_FLAT);
				}
				else
				{
		...
		pShaderAPI->SetPixelShaderConstant(13, vEyePos_SpecExponent, 1);
