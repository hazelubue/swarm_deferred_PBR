...
//fuck this shit for developing right now
...
const float2 g_vecFullScreenTexel		: register( c33 );
...
#if ( DETAIL_BLEND_MODE == 1 ) && ( defined(SHADER_MODEL_PS_2_B ) || defined( SHADER_MODEL_PS_3_0 ) )
	float2 paintCoord;
	if( bBumpmap )
	{
		float2 bumpCoord1;
...
#   endif
	...
	#if ( DETAIL_BLEND_MODE != 1 )
	{
		if( bDetailTexture )
		{
			...
			#if ( ( DETAIL_BLEND_MODE == 0 ) && !MASKEDBLENDING && !BASETEXTURE2 && !SELFILLUM )
			{
...
// ssbump
	...
	if( defint > 0 )
		diffuseComponent = albedo * (min( diffuseLighting, flLighting )); // Darken 
	else
		diffuseComponent = albedo * (diffuseLighting);
