...
float4 GetVolumetricCumulusClouds(
    float cloudAltitude,
    ...
    // Use proper planetary coordinates
    float3 air_viewer_pos = cameraPos + float3(0, planet_radius, 0);
    float r = length(air_viewer_pos);

    // Adjust cloud layer based on cloudAltitude parameter
    float cloud_bottom = planet_radius + cloudAltitude * 1000.0; // Convert km to m
    float cloud_top = cloud_bottom + 2000.0; // 2km thickness

    // Intersect with cloud shell
    float2 dists = intersect_spherical_shell(air_viewer_pos, nPlayerPos, cloud_bottom, cloud_top);

    if (dists.y < 0.0) return float4(0.0, 0.0, 0.0, 0.0);

    float ray_length = min(dists.y - dists.x, 20000.0);
    int sampleCount = clamp(int(ray_length / 150.0), 12, 32);

    float stepMult = ray_length / float(sampleCount);
    float3 traceAdd = nPlayerPos * stepMult;
    float3 tracePos = air_viewer_pos + nPlayerPos * (dists.x + stepMult * dither);

    float transmittance = 1.0;
    float3 scattering = float3(0.0, 0.0, 0.0);
    float cos_theta = dot(nPlayerPos, sunDirection);

    float scattering_coeff = clouds_params.l0_scattering_coeff;
    float extinction_coeff = clouds_params.l0_extinction_coeff;
    [unroll(8)]
    for (int i = 0; i < sampleCount; i++) {
        ...
        float lTracePos = length(tracePos - air_viewer_pos);

        // Use the original density function
        float density = clouds_cumulus_density(tracePos);

        if (density < eps) continue;

        // Distance fade
        float distance_to_sample = length(tracePos - air_viewer_pos);
        density *= smoothstep(1.0, 0.95, distance_to_sample / 20000.0);

        // Lighting
        float step_optical_depth = density * extinction_coeff * stepMult;
        float step_transmittance = exp(-step_optical_depth);

        // Simple light sampling towards sun
        float3 lightSamplePos = tracePos + sunDirection * 500.0;
        float light_density = clouds_cumulus_density(lightSamplePos);
        float light_optical_depth = light_density * extinction_coeff * 500.0;

        float phase = clouds_phase_single(cos_theta);
        float powder = clouds_powder_effect(density, cos_theta);

        // Multi-scatter approximation
        float light_energy = exp(-light_optical_depth) * phase * powder;

        scattering += sunColor * light_energy * scattering_coeff * transmittance * (1.0 - step_transmittance);
        scattering += skyColor * 0.3 * scattering_coeff * transmittance * (1.0 - step_transmittance);

        transmittance *= step_transmittance;

        if (cloudLinearDepth == 0.0 && density > eps) {
            cloudLinearDepth = lTracePos;
        }
    }

    volumetricClouds.rgb = scattering;
    volumetricClouds.a = saturate(1.0 - transmittance);
