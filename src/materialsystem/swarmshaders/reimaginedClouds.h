...
bool GetCloudNoise(float3 tracePos, inout float3 tracePosM, float cloudAltitude, sampler2D noiseTex) {
    ...
    float2 coord2 = baseCoord * 2.0 + tracePosM.y * 0.05;
    float2 coord3 = baseCoord * 4.0 + tracePosM.y * 0.08;
    float2 coord4 = baseCoord * 0.5 - tracePosM.y * 0.02;
    ...
    float adjustedNoise1 = noise1 * 0.7; //g_Coverage1
    float adjustedNoise2 = noise2 * 0.3; //g_Coverage2
    float adjustedNoise3 = noise3 * 0.1; //g_Coverage3 
    float adjustedNoise4 = noise4 * 0.666; //g_Coverage4
    ...
    return density > threshold * 0.6 + 0.7;
}
float4 GetVolumetricClouds(
    ...
    float dither,
    float3 skyColor,
    sampler2D noiseTex  // ADD THIS PARAMETER
) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        if (GetCloudNoise(tracePos, tracePosM, cloudAltitude, noiseTex)) {
            // Calculate shading
