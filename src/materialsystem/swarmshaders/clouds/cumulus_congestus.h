...
#define clouds_cumulus_congestus_radius                 planet_radius + CLOUDS_CUMULUS_CONGESTUS_ALTITUDE
#define clouds_cumulus_congestus_thickness              CLOUDS_CUMULUS_CONGESTUS_ALTITUDE * CLOUDS_CUMULUS_CONGESTUS_THICKNESS
#define clouds_cumulus_congestus_top_radius             clouds_cumulus_congestus_radius + clouds_cumulus_congestus_thickness
#define clouds_cumulus_congestus_distance				30000.0
#define clouds_cumulus_congestus_end_distance			50000.0
#define clouds_cumulus_congestus_extinction_coeff       0.08
#define clouds_cumulus_congestus_scattering_coeff       clouds_cumulus_congestus_extinction_coeff * (1.0 - 0.2 /** rainStrength*/)


// altitude_fracion := 0 at the bottom of the cloud layer and 1 at the top
float clouds_cumulus_congestus_altitude_shaping(float density, float altitude_fracion) {
	// Carve egg shape
	density -= sqr(linear_step(0.3, 1.0, altitude_fracion)) * clamp01(1.0 - 0.2 * clouds_params.cumulus_congestus_blend);

	// Reduce density at the top and bottom of the cloud
	density *= smoothstep(0.0, 0.1, altitude_fracion);
	density *= smoothstep(0.0, 0.1, 1.0 - altitude_fracion);

	return density;
}

float clouds_cumulus_congestus_density(float3 pos) {
	...
	float altitude_fracion = (r - clouds_cumulus_congestus_radius) * rcp(clouds_cumulus_congestus_thickness);
	float distance_fracion = linear_step(clouds_cumulus_congestus_distance, clouds_cumulus_congestus_end_distance, length(pos.xz));

	pos.xz += cameraPosition.xz * CLOUDS_SCALE;

	// 2D noise for base shape and coverage
	float noise = tex2D(noisetex, (0.000003 / CLOUDS_CUMULUS_CONGESTUS_SIZE) * (pos.xz + wind_velocity * (world_age + 50.0 * sqr(altitude_fracion)))).w;
	...
	float density  = 1.5 * sqr(linear_step(0.75 - 0.15 * clamp01(clouds_params.cumulus_congestus_blend * CLOUDS_CUMULUS_CONGESTUS_COVERAGE), 1.0, sqrt(noise)));
	      density  = clouds_cumulus_congestus_altitude_shaping(density, altitude_fracion);
		  density *= 4.0 * distance_fracion * (1.0 - distance_fracion);
	...
	float worley_0 = tex3D(SAMPLER_WORLEY_SWIRLEY, (pos + 0.2 * wind) * 0.00005).x;
	float worley_1 = tex3D(SAMPLER_WORLEY_SWIRLEY, (pos + 0.4 * wind) * 0.00023).x;
#else
	...
	float detail_fade = 0.20 * smoothstep(0.85, 1.0, 1.0 - altitude_fracion)
	                  - 0.35 * smoothstep(0.05, 0.5, altitude_fracion) + 0.8;

	density -= (7.0 * CLOUDS_CUMULUS_CONGESTUS_DETAIL_STRENGTH) * sqr(clamp01(1.0 - density)) * cube(worley_0) * sqr(altitude_fracion);
	density -= (0.2 * CLOUDS_CUMULUS_CONGESTUS_DETAIL_STRENGTH) * sqr(worley_1) * dampen(clamp01(1.0 - density)) * dampen(dampen(clamp01(detail_fade + 0.5 * sqr(altitude_fracion))));

	// Adjust density so that the clouds are wispy at the bottom and hard at the top
	density  = max0(density);
	density  = lift(clamp01(density), mix(2.0, 10.0, altitude_fracion));
	density *= sqr(linear_step(0.0, 0.5, altitude_fracion));
...
float2 clouds_cumulus_congestus_scattering(
	float density,
...
CloudsResult draw_cumulus_congestus_clouds(
	...
	float2 sphere_dists   = intersect_spherical_shell(air_viewer_pos, ray_dir, clouds_cumulus_congestus_radius, clouds_cumulus_congestus_top_radius);
	float2 cylinder_dists = intersect_cylindrical_shell(air_viewer_pos, ray_dir, clouds_cumulus_congestus_distance, clouds_cumulus_congestus_end_distance);
	float2 dists          = float2(max(sphere_dists.x, cylinder_dists.x), min(sphere_dists.y, cylinder_dists.y));
	...
	float2 scattering = (float2)0.0; // x: direct light, y: skylight
	...
	for (uint i = 0u; i < primary_steps; ++i) {
		...
		float altitude_fracion = (length(ray_pos) - clouds_cumulus_congestus_radius) * rcp(clouds_cumulus_congestus_thickness);
		...
		float ground_optical_depth = mix(density, 1.0, clamp01(altitude_fracion * 2.0 - 1.0)) * altitude_fracion * clouds_cumulus_congestus_thickness; // guess optical depth to the ground using altitude fracion and density from this sample
	...
	// Aerial perspective
	float3 clouds_scattering = scattering.x * light_color + scattering.y * (float3)0.1;
