...
float4 GetVolumetricClouds(
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        if (GetCloudNoise(tracePos, tracePosM, cloudAltitude, noiseTex, layerID)) {
            ...
            float step_optical_depth = density * stepMult;
            ...
            float cos_theta = dot(nPlayerPos, directColor);

            float3 lightSamplePos = tracePos + directColor * 500.0;
            float light_density = clouds_cumulus_density(lightSamplePos);
            float light_optical_depth = light_density * 500.0;
            ...
            scattering += directColor * light_energy * transmittance * (1.0 - step_transmittance);
            scattering += skyColor * 0.3  * transmittance * (1.0 - step_transmittance);
            ...
            colorSample = lerp(scattering, colorSample, cloudFogFactor);
