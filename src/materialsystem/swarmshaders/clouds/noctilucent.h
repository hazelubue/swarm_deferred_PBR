...
float clouds_noctilucent_density(float2 coord, float3 ray_dir) {
	coord *= 0.25;
	...
	float density = dampen(tex2D(noisetex, 0.000001 * coord + 0.02 * curl).y);
	...
	for (int i = 0; i < 4; ++i) {
		float detail = tex2D(noisetex, coord * detail_frequency + curl * curl_strength).y;
	...
	float highlight  = dampen(tex2D(noisetex, 0.000004 * coord + 0.02 * curl).y);
	      highlight -= (1.0 - tex2D(noisetex, 0.000014 * coord + 0.05 * curl).y) * 0.15;

	density += 2.0 * pow8(max0(highlight));

	return sqr(max0(density)) * 0.344/*CloudsParameters.noctilucent_amount*/;
}

float4 draw_noctilucent_clouds(
	float3 air_viewer_pos,
	float3 ray_dir,
	float3 sun_dir,
	float3 clear_sky
) {
	const float3  color            = 0.35 * float3(1.5, 2.0, 9.0);
	const float phase_g          = 0.8;
	const float extinction_coeff = 0.001; 

	float visibility = pulse(sun_dir.y, -0.10, 0.1);

	if (visibility < eps || 0.344/*clouds_params.noctilucent_amount*/ < eps) {
	...
	float2 dists = intersect_sphere(air_viewer_pos, ray_dir, clouds_noctilucent_radius);
	...
	float3 scattering = visibility * color * density * phase;
