	...
	VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData* pDeferredContext)
{
		...
		DYNAMIC_STATE
	{
		...
		int totalLightCount = pExt->GetNumActiveLights_Simple();

		float totalLightCountFloat = (float)totalLightCount;
		pShaderAPI->SetPixelShaderConstant(48, &totalLightCountFloat, 1);

		float rowsPerLight = 4.0f; 
		pShaderAPI->SetPixelShaderConstant(47, &rowsPerLight, 1);

		ITexture* pLightDataTex = pExt->GetTexture_ForwardData();
		float textureHeight = (float)pLightDataTex->GetActualHeight();
		pShaderAPI->SetPixelShaderConstant(46, &textureHeight, 1);

		float dataRows = (float)pExt->m_iCommon_NumRows;
		pShaderAPI->SetPixelShaderConstant(49, &dataRows, 1);

		pShader->BindTexture(SHADER_SAMPLER13, pExt->GetTexture_ForwardData());
		...
		pShaderAPI->SetPixelShaderConstant(38, data.matCorrectedViewProj.Base(), 4);
		pShaderAPI->SetPixelShaderConstant(42, data.matCorrectedViewProjInv.Base(), 4);
