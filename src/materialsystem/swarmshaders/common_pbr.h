...
//{
//    return F0 + (max(1.0f.xxx - F0, F0) - F0) * pow(1.0f - cosTheta, 5.0f) * roughness;
//}
//
//float3 DoIBL(float3 N, float3 V, float3 albedo, float roughness, float3 F0, float3 ambient)
//{
//    N = normalize(N);
//    V = normalize(V);
//
...
//
//    float3 kd = (1.0f - F) * (1.0f - 0.9); // assume low metalness for ambient
//
//    float3 diffuse = kd * albedo * ambient;
//
//    // Specular ambient approximation (simplified)
//    float3 specular = F * ambient * 0.5;
//
