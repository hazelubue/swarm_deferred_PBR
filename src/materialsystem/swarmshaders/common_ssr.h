...
float4x4 GetViewProj_frustum()
{
    //return 0;
    return mul(frustum_ViewMatrix(), g_Proj);
}
...
float3 performIBL_screespace(float2 uv, float3 worldNormals, float roughness, float3 view, sampler input_sampler, sampler input_depth, float depthMask)
{
    ...
    while (j++ < maxIterations)
    {
        ...
        float3 RayWorldPos = readWorld(WorldToUV(iblRayPos.xyz), input_depth);
        //make projectable uv.
        float2 iblScreen = WorldToUV(iblRayPos.xyz);
        iblUV = iblScreen.xy;
...
float3 ssrComposition_simple(float roughness, float metallic, float2 uv, float3 N, sampler input_sampler, sampler input_depth, float4 worldPos, float3 view)
{
    ...
    float depth_compare = readDepth_Normalized(WorldToView(RayCurPos.xyz), input_depth) + 0.1f;
    ...
    while (j++ < maxIterations)
    {
        ...
        for (int i = 0; i < refinementSteps; i++)
        {
            ...
            depth_compare = readDepth_Normalized(WorldToView(RayCurPos.xyz), input_depth);
    ...
    return float3(tappedSSRresult * fresnelMask * UVmask * mask * depthMask);
}
