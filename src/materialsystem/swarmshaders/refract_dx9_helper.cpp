...
#include "convar.h"
#include "include/refract_vs30.inc"
#include "include/refract_ps30.inc"
...
void InitParamsRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, Refract_DX9_Vars_t &info )
{
	...
	if (!params[info.m_nAlbedo]->IsDefined())
	{
		params[info.m_nAlbedo]->SetStringValue("nature/water_lake");
	}
...
void InitRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, Refract_DX9_Vars_t &info )
{
	...
	if (params[info.m_nAlbedo]->IsDefined())
	{
		pShader->LoadTexture(info.m_nAlbedo);
	}
}
					 ...
					 IShaderShadow* pShaderShadow, Refract_DX9_Vars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	DYNAMIC_STATE
	{
		...
		if( params[info.m_nAlbedo]->IsTexture())
		{
			pShader->BindTexture(SHADER_SAMPLER6, info.m_nAlbedo);
		}
		...
		pShader->SetPixelShaderConstant( 14, info.m_nEnvmapSaturation );
