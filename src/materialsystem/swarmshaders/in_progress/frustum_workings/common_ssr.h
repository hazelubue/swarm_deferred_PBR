...
float2 WorldToUV(in float3 position, float3 frustumRay)
{
    float4x1 viewProj = mul(float4(frustumRay, 1.0), g_Proj);
    float4 viewPos = mul(float4(position, 1.0), viewProj);
...
float3 WorldToView(in float3 position, float3 frustumRay)
{
    float4x1 viewProj = mul(float4(frustumRay, 1.0), g_Proj);
    float4 viewPos = mul(float4(position, 1.0), viewProj);
    return viewPos.xyz;
...
float3 WorldToScreen(in float3 Position)
{
    float4 clipPos = mul(float4(Position, 1.0f), g_Proj);
    clipPos.xyz /= clipPos.w;

    float2 screenPos = (clipPos.xy * 0.5 + 0.5);

    return float3(screenPos, 0.0);
}
...
float3 performIBL_screespace(float2 uv, float3 worldNormals, float roughness, float3 view, sampler input_sampler, sampler input_depth, float depthMask)
{
    ...
    float3 V = normalize(-view);
        ...
        {
            frustumRay = readWorld(uv, input_depth);

            //convert to ray in world pos
            float3 RayWorldPos = readWorld(WorldToUV(iblRayPos.xyz, frustumRay), input_depth);
            //make projectable uv.
            float2 iblScreen = WorldToUV(iblRayPos.xyz, frustumRay);
            iblUV = iblScreen.xy;
...
float3 ssrComposition_simple(float roughness, float metallic, float2 uv, float3 N, sampler input_sampler, sampler input_depth, float4 worldPos, float3 view)
{
    ...
    float depth_compare = readDepth_Normalized(WorldToView(RayCurPos.xyz, frustumRay), input_depth) + 0.1f;
    ...
    while (j++ < maxIterations)
    {
        ...
        float3 RayWorldPos = readWorld(WorldToUV(RayCurPos.xyz, frustumRay), input_depth);

        rayScreen = WorldToUV(RayCurPos.xyz, frustumRay);
        ...
        for (int i = 0; i < refinementSteps; i++)
        {
            ...
            depth_compare = readDepth_Normalized(WorldToView(RayCurPos.xyz, frustumRay), input_depth);
    ...
    return float3(tappedSSRresult * UVmask * mask * depthMask) * fresnelMask;
}
