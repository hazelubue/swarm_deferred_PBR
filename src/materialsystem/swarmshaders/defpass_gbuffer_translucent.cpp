	...
	VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData* pDeferredContext)
{
	...
	if (pDeferredContext->m_bMaterialVarsChanged || !pDeferredContext->HasCommands(CDeferredPerMaterialContextData::DEFSTAGE_GBUFFER_translucent))
	{
		tmpBuf.Reset();
		...
		pDeferredContext->SetCommands(CDeferredPerMaterialContextData::DEFSTAGE_GBUFFER_translucent, tmpBuf.Copy());
	...
	SET_DYNAMIC_VERTEX_SHADER(gbuffer_vs30);
		...
		pShaderAPI->ExecuteCommandBuffer(pDeferredContext->GetCommands(CDeferredPerMaterialContextData::DEFSTAGE_GBUFFER_translucent));
