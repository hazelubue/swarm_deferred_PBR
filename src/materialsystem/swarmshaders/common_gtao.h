...
float compute_maximum_horizon_angle(
	...
	float2 ray_step = (view_to_screen_space(view_pos + view_slice_dir * step_size, true) - screen_pos).xy;
	float2 ray_pos = screen_pos.xy + ray_step * (dither + max_of(view_pixel_size) * rcp_length(ray_step));

	for (int i = 0; i < GTAO_HORIZON_STEPS; ++i, ray_pos += ray_step) {
		ifloat2 texel = ifloat2(clamp01(ray_pos) * view_res * taau_render_scale - 0.5);
		float depth = texelFetch(combined_depth_buffer, texel, 0).x;

		if (depth == 1.0 || depth < hand_depth || depth == screen_pos.z) continue;

		float3 offset = screen_to_view_space(combined_projection_matrix_inverse, float3(ray_pos, depth), true) - view_pos;
...
float2 compute_gtao(
	...
	bent_normal = float3(0.0);

	// Construct local working space
	float3 viewer_dir = normalize(-view_pos);
	...
	for (int i = 0; i < GTAO_SLICES; ++i) {
		...
		float3 view_slice_dir = local_to_view * slice_dir;
		...
		max_horizon_angles.x = compute_maximum_horizon_angle(-view_slice_dir, viewer_dir, screen_pos, view_pos, ao_radius, dither.y);
		max_horizon_angles.y = compute_maximum_horizon_angle(view_slice_dir, viewer_dir, screen_pos, view_pos, ao_radius, dither.y);

		ambient_sss += max0(max_horizon_angles.y - half_pi) * rcp_pi;

		max_horizon_angles = gamma + clamp(float2(-1.0, 1.0) * max_horizon_angles - gamma, -half_pi, half_pi);
	...
	return clamp01(float2(ao, ambient_sss));
}
