...
extern ConVar fog_hdrcolorscale;
extern ConVar fog_hdrcolorscaleskybox;

extern void FogOverrideCallback(IConVar* pConVar, char const* pOldString, float flOldValue);
...
Vector4D MultiplyMatrixVector(const VMatrix& mat, const Vector4D& vec)
{
	Vector4D out;
	Vector4DMultiply(mat, vec, out);
	return out;
}

class CWaterViewCapture
{
private:
	ITexture* m_pRenderTargetCopy;
	unsigned char* m_pPixelBuffer;
	CViewSetup* m_pViewBuffer;
	int m_iWidth, m_iHeight;
	bool m_bFrameReady;

public:
	CWaterViewCapture() :
		m_pRenderTargetCopy(NULL),
		m_pPixelBuffer(NULL),
		m_pViewBuffer(NULL),
	...
	~CWaterViewCapture()
	{
		if (m_pPixelBuffer) delete[] m_pPixelBuffer;
		if (m_pViewBuffer) delete[] m_pViewBuffer;
	}

	bool Init()
	{
		// Use a reasonable fixed size or get screen dimensions directly
		...
		char rtName[128];
		static int s_RTIndex = 0;
		sprintf(rtName, "_WaterViewCopy_%d", s_RTIndex++);

		m_pRenderTargetCopy = materials->CreateNamedRenderTargetTextureEx2(
			rtName,
			m_iWidth, m_iHeight,
			RT_SIZE_NO_CHANGE,
		...
		if (!m_pRenderTargetCopy)
			return false;

		m_pRenderTargetCopy->IncrementReferenceCount();

		m_pPixelBuffer = new unsigned char[m_iWidth * m_iHeight * 4];
		m_pViewBuffer = new CViewSetup[m_iWidth * m_iHeight];

		Warning("CWaterViewCapture::Init - Size: %dx%d\n", m_iWidth, m_iHeight);

		return true;
	}

	void CaptureFrame(const CViewSetup& mainView)
	{
		if (!m_pRenderTargetCopy || !g_pMaterialSystem)
			return;

		CMatRenderContextPtr pRenderContext(materials);

		ITexture* pFullFrameFB = materials->FindTexture("_rt_FullFrameFB", TEXTURE_GROUP_RENDER_TARGET);
		ITexture* pDepthRT = materials->FindTexture("_rt_FullFrameDepth", TEXTURE_GROUP_RENDER_TARGET);

		if (!pFullFrameFB || !pDepthRT) return;

		pRenderContext->CopyRenderTargetToTexture(pFullFrameFB);

		pRenderContext->PushRenderTargetAndViewport(pFullFrameFB);
		pRenderContext->ReadPixels(0, 0, m_iWidth, m_iHeight,
			m_pPixelBuffer, IMAGE_FORMAT_RGBA8888);
		pRenderContext->PopRenderTargetAndViewport();

		unsigned char* pDepthPixels = new unsigned char[m_iWidth * m_iHeight * 4];
		pRenderContext->PushRenderTargetAndViewport(pDepthRT);
		pRenderContext->ReadPixels(0, 0, m_iWidth, m_iHeight,
			pDepthPixels, IMAGE_FORMAT_RGBA8888);
		pRenderContext->PopRenderTargetAndViewport();

		ConvertToViews(m_pPixelBuffer, pDepthPixels,
			mainView.origin,
			mainView.angles,
			mainView.fov,
			(float)m_iWidth / m_iHeight,
			mainView.zNear,
			mainView.zFar);
	...
	void ConvertToViews(unsigned char* pPixels,
		unsigned char* pDepthPixels,
		const Vector& cameraOrigin,
		const QAngle& cameraAngles,
		float fov,
		float aspectRatio,
		float zNear,
		float zFar)
	{
		...
		AngleVectors(cameraAngles, &cameraForward, &cameraRight, &cameraUp);

		float tanHalfFov = tan(DEG2RAD(fov * 0.5f));
		float tanHalfFovX = tanHalfFov * aspectRatio;

		CViewSetup& view = m_pViewBuffer[0];

		float screenX = 0.0f;
		float screenY = 0.0f;

		Vector rayDir = cameraForward +
			(cameraRight * screenX * tanHalfFovX) +
			(cameraUp * screenY * tanHalfFov);
		VectorNormalize(rayDir);

		float depth = 1.0f;
		view.origin = cameraOrigin + rayDir * depth;

		VectorAngles(rayDir, view.angles);

		view.fov = fov;
		view.m_flAspectRatio = aspectRatio;
	}

	CViewSetup* GetViews(int& width, int& height)
	{
		if (!m_bFrameReady) return NULL;

		width = m_iWidth;
		height = m_iHeight;
		m_bFrameReady = false;

		return m_pViewBuffer;
	}
};
...
static inline unsigned long BuildEngineDrawWorldListFlags(unsigned nDrawFlags)
{
	...
	if (nDrawFlags & DF_SKIP_NODRAW)
	{
		nEngineFlags |= DRAWWORLDLISTS_DRAW_SKIP_NODRAW;
	}
...
void CDeferredViewRender::UpdateIndependentViewports()
{
	C_BasePlayer* pPlayer = C_BasePlayer::GetLocalPlayer();
	if (pPlayer)
	{
		pPlayer->UpdateIndependentViewport();
	}
}
	...
	SkyboxVisibility_t& nSkyboxVisible, bool bDrawViewModel)
{
	...
	//pRenderContext->SetRenderTargetEx(1, GetDefRT_WaterNormals());
...
void CDeferredViewRender::ViewDrawSceneDeferred(const CViewSetup& view, int nClearFlags, view_id_t viewID, bool bDrawViewModel)
{
	...
	CMatRenderContextPtr pRenderContext(materials);
	/*pRenderContext->CopyRenderTargetToTextureEx(GetFullFrameDepthTexture(), 0, NULL, NULL);
	...
	//pLightMan->CommitLightDataTexture();
...
CWaterViewCapture g_WaterViewCapture;

void InitWaterViewCapture()
{
	g_WaterViewCapture.Init();
}

void CDeferredViewRender::PerformMatrixView(const CViewSetup& view)
{
	CViewSetup ReflectionView = view;
	ReflectionView.origin = g_vecRenderOriginStatic;
	ReflectionView.angles = g_vecRenderAnglesStatic;

	//adjust fov by an elipson to add a slight offset.
	ReflectionView.fov = view.fov * 1.000001f;
	//create declaration of reflection view.
	QAngle ReflectionAngle = ReflectionView.angles;

	Vector xVec(ReflectionAngle.x, ReflectionAngle.y, ReflectionAngle.z);
	QAngle AdjAngle(xVec.x, xVec.y, xVec.z);

	//AdjAngle = - AdjAngle;

	//calculate our offset view
	Vector adjVector(AdjAngle.x, ReflectionAngle.y, ReflectionAngle.z);
	//use it
	QAngle finalAng(adjVector.x, adjVector.y, adjVector.z);
	
	//use the final angle
	ReflectionView.angles = finalAng;



	ProcessGlobalMatrixData(ReflectionView);
}
...
void CDeferredViewRender::RenderView(const CViewSetup& view, const CViewSetup& hudViewSetup, int nClearFlags, int whatToDraw)
{
	...
	g_WaterViewCapture.CaptureFrame(view);
	...
	if (pPlayer)
	{
		if (!pPlayer->IsIndependentViewportEnabled())
		{
			pPlayer->EnableIndependentViewport(true);
		}

		pPlayer->UpdateIndependentViewport();

		Vector eyeOrigin, SSReyeOrigin;
		QAngle eyeAngles, SSReyeAngles;
		float zNear, zFar, fov;

		pPlayer->GetIndependentViewport(eyeOrigin, eyeAngles, SSReyeOrigin, SSReyeAngles, zNear, zFar, fov);

		Warning("[Deferred] Independent viewport data: Pos(%.1f,%.1f,%.1f) FOV:%.1f\n",
			eyeOrigin.x, eyeOrigin.y, eyeOrigin.z, fov);

		CViewSetup adjView = view;

		adjView.angles = eyeAngles;
		adjView.origin = eyeOrigin;

		//PerformMatrixView(adjView);
	}
...

	{
		...
		//CViewSetup oldView = GetOriginalViewSetup(&g_OriginalViewSetup);

		/*IViewRender* pViewRender = GetViewRenderInstance();*/
		//const CViewSetup* pViewSetup = GetDeferredViewSetup(&oldView);
		...
		def_light_t* l = nullptr;
		l = new def_light_t;

		GetDeferredExt()->InitIndexTexture();
		GetDeferredExt()->UpdateIndexTexture(l);

		GetLightingManager()->IndexTextureData();
...
//{
//	VMatrix matView, matProj, matViewProj, matViewProjInv, one, two;
...
void CDeferredViewRender::ProcessGlobalMatrixData(const CViewSetup& view)
{
	CViewSetup ssrView = view;

	VMatrix matProj, matViewProj, matViewProjInv, matStaticViewProj, matStaticViewProjInv, one;
	render->GetMatricesForView(ssrView, &one, &matProj, &matViewProj, &matViewProjInv);

	VMatrix matView = CurrentWorldToViewMatrix();
	Vector eye;

	eye.x = view.fov * cos(view.angles.x) * cos(view.angles.y);
	eye.y = view.fov * sin(view.angles.x);
	eye.z = view.fov * cos(view.angles.x) * sin(view.angles.y);

	Vector F = -CurrentViewForward();
	Vector R = CurrentViewRight();
	Vector U = -CurrentViewUp();

	/*VMatrix matView = VMatrix(
		R.x, U.x, F.x, 0,
		R.y, U.y, F.y, 0,
		R.z, U.z, F.z, 0,
		-eye.x, -eye.y, -eye.z, 1
	...
	float viewportOffsetX = (float)view.x;
	float viewportOffsetY = (float)view.y;
...
//void CDeferredViewRender::ProcessGlobalMatrixData(const CViewSetup& view)
//{
//	VMatrix matView, matProj, matViewProj, matViewProjInv, matStaticViewProj, matStaticViewProjInv;
//	render->GetMatricesForView(view, &matView, &matProj, &matViewProj, &matViewProjInv);
//
//	
//
//	Vector D, R, U;
//	AngleVectors(view.angles, &D, &R, &U);
//	Vector P = -view.origin;
//	VMatrix mFirst;
//	mFirst.Init(
//		R.x, R.y, R.z, 0,
//		U.x, U.y, U.z, 0,
...
//	mFirst = mFirst * mSecond;
//
//	// Build static ViewProj
//	MatrixMultiply(mFirst, matProj, matStaticViewProj);
//	MatrixInverseGeneral(matStaticViewProj, matStaticViewProjInv);
//
//	VMatrix matViewInv, matProjInv;
//	MatrixInverseGeneral(matView, matViewInv);
//	MatrixInverseGeneral(matProj, matProjInv);
...
//		matProjInv,
//		matViewProjInv,
//		matStaticViewProj,      // correctedViewProj
//		matStaticViewProjInv    // correctedViewProjInv
//	);
//}
...
static bool ShouldDrawSurface(IClientEntity* pBaseEntity, const model_t* pModel, int nSurfFlags, int nDrawFlags)
{
	// Skip nodraw surfaces when flag is set
	if ((nDrawFlags & DF_SKIP_NODRAW) && (nSurfFlags & SURF_NODRAW))
	{
		return false;
	}
...
void CGBufferView::Setup(const CViewSetup& view, bool bDrewSkybox, const WaterRenderInfo_t& waterInfo)
{
...
void CGBufferViewTranslucent::PushGBufferTranslucent(bool bInitial, float zScale, bool bClearDepth)
{
    static ITexture* pNormals = GetDefRT_Normals();
    static ITexture* pDepth = GetDefRT_Depth();
    static ITexture* pLightCtrl = GetDefRT_LightCtrl();
    
    CMatRenderContextPtr pRenderContext(materials);
    
    if (bInitial || bClearDepth)
    {
        pRenderContext->PushRenderTargetAndViewport(pDepth);
        pRenderContext->ClearColor4ub(0, 0, 0, 255);
    ...
    pRenderContext->PushRenderTargetAndViewport(pNormals);
    pRenderContext->ClearColor4ub(0, 0, 0, 125);
    pRenderContext->ClearBuffers(true, false, false);
    pRenderContext->PopRenderTargetAndViewport();
    
    pRenderContext->PushRenderTargetAndViewport(pLightCtrl);
    pRenderContext->ClearColor4ub(0, 0, 0, 125);
    pRenderContext->ClearBuffers(true, false, false);
    pRenderContext->PopRenderTargetAndViewport();
    ...
    pRenderContext->PushRenderTargetAndViewport(pNormals);
    pRenderContext->SetRenderTargetEx(1, pDepth);
    pRenderContext->SetRenderTargetEx(3, pLightCtrl);
...
void CBaseWorldViewDeferred::PushView(float waterHeight)
{
	...
	//if (m_DrawFlags & DF_DRAW_ENTITITES)  // Composition pass
	//{
	//	pRenderContext->SetRenderTargetEx(1, GetDefRT_WaterNormals());
	//}
...
void CBaseWorldViewDeferred::DrawExecute(float waterHeight, view_id_t viewID, float waterZAdjust, bool bShadowDepth)
{
	...
	if (m_DrawFlags & DF_SKIP_NODRAW)
	{
		// This is a hack: nodraw surfaces won't render because they'll be overridden with nothing
		// The engine will still process them but they won't draw
		// This is not ideal but works without engine access
	}
...
void CBaseWorldViewDeferred::PushComposite()
{
	...
	//ITexture* pSSR = GetDefRT_WaterNormals();
	//pRenderContext->SetRenderTargetEx(1, pSSR);
}
...
void CBaseShadowView::Setup(const CViewSetup& view, ITexture* pDepthTexture, ITexture* pDummyTexture)
{
	...
	m_DrawFlags = DF_DRAW_ENTITITES | DF_RENDER_UNDERWATER | DF_RENDER_ABOVEWATER | DF_SKIP_NODRAW;
	m_ClearFlags = 0;
...
//random ahh place after everything is drawn
void PostRender_Water(const CViewSetup& currentView)
{
	g_WaterViewCapture.CaptureFrame(currentView);
}
