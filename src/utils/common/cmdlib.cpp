...
class CmdLib_SpewOutputFunc : public ILoggingListener
{
public:
	void Log( const LoggingContext_t *pContext, const tchar *pMsg ) override
	{
		...
		{
			if ( ( pContext->m_Severity == LS_MESSAGE ) )
			{
				Color c = pContext->m_Color;
			...
			else if ( pContext->m_Severity == LS_ASSERT )
			{
...
void InstallSpewFunction()
{
	...
	LoggingSystem_RegisterLoggingListener( &g_CmdLib_SpewOutputFunc );
	GetInitialColors();
}
